...
const AccountBorrowTable: React.FC<IProps> = () => {
  ...
  const marketId = marketVm.market?.contractAddress ?? "";
    ...
    const data = [].map((s: TMarketStats) => ({
      ...
      repayBtn: (
        <Button
          ...
          size="medium"
          fixed
          onClick={(e) => openModal(e, marketId, "repay", s.assetId)}
          style={{ width: "100px", margin: "0 auto" }}
        >
          Repay
        </Button>
  ...
  }, [vm.sortBorrow, vm.sortModeBorrow, marketVm, openModal, navigate]);
