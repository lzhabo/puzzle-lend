...
import { TPoolStats } from "@stores/LendStore";
import BN from "@src/utils/BN";

const ctx = React.createContext<TotalRewardsVM | null>(null);
...
export const TotalRewardsVMProvider: React.FC<IProps> = ({ children }) => {
  const rootStore = useStores();
  const store = useMemo(() => new TotalRewardsVM(rootStore), [rootStore]);
  return <ctx.Provider value={store}>{children}</ctx.Provider>;
};

export const useTotalRewardsVM = () => useVM(ctx);

class TotalRewardsVM {
